...
test('nextRankNeed: みならい→3・いっちょまえ→6・でんせつ→9・まぼろし→null', () => {
  ...
  assert.equal(nextRankNeed(3, CONFIG), CONFIG.UNLOCK.MABOROSHI_STREAK)
  assert.equal(nextRankNeed(4, CONFIG), null)
})
...
test('rankInfo: ランク名と残り回数', () => {
  ...
  assert.equal(r3.needed, CONFIG.UNLOCK.MABOROSHI_STREAK)
  assert.equal(r3.remaining, 1)
  assert.equal(r3.nextName, 'まぼろしの砲手')
})
